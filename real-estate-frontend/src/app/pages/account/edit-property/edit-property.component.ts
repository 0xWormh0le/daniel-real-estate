...
export class EditPropertyComponent implements OnInit {
  ...
  public getPropertyById(id){
    this.appService.getPropertyById(id).subscribe(data=>{
      ...
      this.submitForm.controls.basic.get('priceDollarSale').setValue((this.property.details.price_dollar_sale)?this.property.details.price_dollar_sale:this.property.details.price_dollar_rent);
      this.submitForm.controls.basic.get('priceDollarRent').setValue((this.property.details.price_dollar_rent)?this.property.details.price_dollar_rent:this.property.details.price_dollar_rent);
      this.submitForm.controls.basic.get('propertyType').setValue( this.propertyTypes.filter(p => p.name == this.property.type)[0]);
      
      const statusList: any[] = []; 
      this.propertyStatuses.forEach(status =>{
        this.property.status.forEach(name=>{
          if(status.name == name){
      ...
      this.submitForm.controls.address.get('location').setValue(this.property.formatted_address);  
      this.submitForm.controls.address.get('city').setValue(this.property.details.city);  
      this.submitForm.controls.address.get('street').setValue(this.property.details.street);  
      this.lat = this.property.details.lat;
      this.lng = this.property.details.lng; 
      this.getAddress();
       
      this.submitForm.controls.additional.get('bedrooms').setValue(this.property.details.bedrooms);  
      this.submitForm.controls.additional.get('bathrooms').setValue(this.property.details.bathrooms);  
      this.submitForm.controls.additional.get('garages').setValue(this.property.details.garages);  
      this.submitForm.controls.additional.get('area').setValue(this.property.details.area.value);
      this.submitForm.controls.additional.get('yearBuilt').setValue(this.property.details.year);
      ...
      this.property.additional_features.forEach(feature => additionalFeatures.push(this.fb.group(feature)));
          ...
          this.submitForm.value.media.videos.forEach(e => {
            ...
            if (e.link.trim() != '') {
              this.appService.submitAttachment(video).then(attachResult => {
                ...
                videoPromises.push(this.appService.submitGallery(this.property.id, video));
              })
            } else {
              videoPromises.push(this.appService.submitGallery(this.property.id, video));
            }
